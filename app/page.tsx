...
/* ── Large flower SVG shapes for background (not emoji) ── */
function FlowerBg({ mx, my }: { mx: number; my: number }) {
  ...
  const flowers = [
    { cx: 0.08, cy: 0.12, r: 220, color: "#F9C846", petals: 5, rot: 0,   delay: "0s",   dur: "14s"  },
    { cx: 0.88, cy: 0.08, r: 200, color: "#4BAF7E", petals: 6, rot: 20,  delay: "-5s",  dur: "18s"  },
    { cx: 0.04, cy: 0.62, r: 180, color: "#E8435A", petals: 5, rot: 45,  delay: "-9s",  dur: "12s"  },
    { cx: 0.92, cy: 0.70, r: 190, color: "#8B6EE8", petals: 6, rot: 10,  delay: "-3s",  dur: "16s"  },
    { cx: 0.48, cy: 0.95, r: 210, color: "#F07048", petals: 5, rot: 72,  delay: "-7s",  dur: "20s"  },
    { cx: 0.65, cy: 0.35, r: 140, color: "#2EBFAC", petals: 6, rot: 30,  delay: "-11s", dur: "15s"  },
    { cx: 0.22, cy: 0.88, r: 150, color: "#F9C846", petals: 5, rot: 15,  delay: "-2s",  dur: "17s"  },
  ];

  // Build petal path for a flower centred at 0,0
  const petalPath = (petals: number, r: number) => {
    const pr = r * 0.52;  // petal radius
    const pc = r * 0.28;  // centre offset
    let d = "";
    for (let i = 0; i < petals; i++) {
      const angle = (i / petals) * Math.PI * 2;
      const ox = Math.cos(angle) * pc;
      const oy = Math.sin(angle) * pc;
      const rx = pr;
      const ry = pr * 0.55;
      const rot = (angle * 180) / Math.PI;
      d += `<ellipse cx="${ox.toFixed(1)}" cy="${oy.toFixed(1)}" rx="${rx}" ry="${ry}" transform="rotate(${rot.toFixed(1)} ${ox.toFixed(1)} ${oy.toFixed(1)})" fill="currentColor" />`;
    }
    return d;
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden" }}>
      {flowers.map((f, i) => {
        const depth = 0.04 + i * 0.008;
        const x = f.cx * 100 + px * 4 * (i % 3 === 0 ? 1 : -0.8);
        const y = f.cy * 100 + py * 4 * (i % 2 === 0 ? 1 : -0.7);
        return (
          <div
            key={i}
            style={{
              ...
              height: f.r * 2,
              opacity: 0.72,
              animation: `flowerSpin ${f.dur} linear infinite`,
          ...
          >
            <svg
              viewBox={`${-f.r} ${-f.r} ${f.r * 2} ${f.r * 2}`}
              width={f.r * 2}
              height={f.r * 2}
              style={{ overflow: "visible" }}
            >
              <g color={f.color} transform={`rotate(${f.rot})`}>
                <g dangerouslySetInnerHTML={{ __html: petalPath(f.petals, f.r) }} />
                <circle cx="0" cy="0" r={f.r * 0.2} fill="white" opacity=".65" />
                <circle cx="0" cy="0" r={f.r * 0.1} fill={f.color} opacity=".5" />
              </g>
            </svg>
          </div>
        );
...
export default function Home() {
  ...
  /* ── MOUSE TRACKING for flower parallax ── */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll<HTMLElement>(".card").forEach(c => {
      ...
      const obs = new IntersectionObserver(entries => {
        entries.forEach(e => { if (e.isIntersecting) { setTimeout(() => e.target.classList.add("in"), ms); obs.unobserve(e.target); } });
      }, { threshold: 0.05 });
      obs.observe(c);
    });
  ...
  return (<>
    <style>{`
      ...
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      /* ── FLOWER SPIN ── */
      @keyframes flowerSpin { from { transform: translate(-50%,-50%) rotate(0deg); } to { transform: translate(-50%,-50%) rotate(360deg); } }
      ...
      /* ── HERO NAME — nowrap so it stays on one line ── */
      .hero-name {
      ...
      /* Dashed ring — purely decorative, does NOT clip anything */
      .orbit-ring-el {
        position:absolute;
        top:50%; left:50%;
        ...
        pointer-events:none;
        animation:spinRingA 22s linear infinite;
      }
      @keyframes spinRingA { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }

      /* Pill wrapper rotates; pills counter-rotate to stay upright */
      .pill-wrap {
        position:absolute;
        top:50%; left:50%;
        width:340px; height:340px;
      ...
      .opill:nth-child(1) {
        ...
        animation: counterN 22s linear infinite;
      ...
      .opill:nth-child(2) {
        ...
        animation: counterE 22s linear infinite;
      ...
      /* Counter-rotations keep text upright */
      ...
      /* Section word titles — full FILLED, with spacing above */
      .div-wrap { padding:80px 72px 40px; max-width:1240px; margin:0 auto; position:relative; z-index:2; }
      .div-word { font-family:'Boldonse',cursive; font-size:clamp(52px,8vw,110px); line-height:.95; letter-spacing:-3px; color:var(--ink); user-select:none; }
      ...
      .site-link {
        ...
        font-size:12px; font-family:'DM Mono',monospace;
        color:var(--sage); background:rgba(75,175,126,.1);
        border:1.5px solid rgba(75,175,126,.28); border-radius:100px;
        padding:5px 12px; text-decoration:none;
        transition:transform .2s,background .2s;
      }
      .site-link:hover { transform:translateY(-2px); background:rgba(75,175,126,.2); }

      /* Flores flower visual */
      ...
      .skill-section-label { font-family:'Boldonse',cursive; font-size:22px; color:var(--ink); margin-bottom:18px; display:flex; align-items:center; gap:12px; }
      ...
      /* Multi-colour big phrase */
      .c-big { font-family:'Boldonse',cursive; font-size:clamp(34px,5.5vw,68px); line-height:.95; letter-spacing:-2px; margin-bottom:16px; position:relative; z-index:1; }
      .c-w1 { color:var(--rose); }
      .c-w2 { color:var(--peach); }
      .c-w3 { color:var(--sun); }
      .c-w4 { color:var(--sage); }
      .c-w5 { color:var(--sky); }
      .c-w6 { color:var(--lav); }
    ...
    <div className="hero">
      ...
      {/* RIGHT — photo with orbiting pills */}
      <div className="hero-right">
        <div className="orbit-container">
          <div className="orbit-ring-el" />

          {/* Pills at z-index 20 — above photo */}
          ...
          {/* Photo at z-index 10 — behind pills */}
    ...
    <section className="sec" style={{paddingTop:0}}>
      ...
      <div className="grid2">
        ...
        {/* Flores — rich card with clickable links */}
        ...
        <div className="card cr" data-d="80">
          ...
          <div className="wtags"><span className="tag tr">Youth Tech</span><span className="tag tp">Coaching</span><span className="tag tl">Robotics</span></div>
        </div>
    ...
    <section className="sec">
      ...
      <div className="sec-title rev" style={{marginBottom:"40px"}}>Languages.</div>
    ...
    <div className="contact">
      {/* Colourful multi-word phrase */}
      <div className="c-big rev">
        <span className="c-w1">The </span>
        <span className="c-w2">best </span>
        <span className="c-w3">projects </span>
        <span className="c-w4">start </span>
        <span className="c-w5">with </span>
        <span className="c-w1">a </span>
        <span className="c-w6">conversation.</span>
      </div>
